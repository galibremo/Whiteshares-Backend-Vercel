...
import { AccountSchemaType, ProfileSchemaType, UserSchemaType } from "@/databases/drizzle/types";
...
export default class AuthenticationService extends DrizzleService {
	...
	async checkUserExistence(id: number): Promise<ServiceApiResponse<boolean>> {
		try {
			const user = await this.db.query.users.findFirst({
				where: eq(users.id, id)
			});
			...
			return ServiceResponse.createResponse(status.HTTP_200_OK, "User exists", true);
		} catch (error) {
			return ServiceResponse.createErrorResponse(error);
		}
	}
	...
	async retrieveUserProfile(id: number): Promise<ServiceApiResponse<ProfileSchemaType>> {
		try {
	...
	async updateUserProfile(
		id: number,
		data: Partial<ProfileSchemaType>
	): Promise<ServiceApiResponse<boolean>> {
		try {
			...
			return ServiceResponse.createResponse(status.HTTP_200_OK, "User updated successfully", true);
		} catch (error) {
			return ServiceResponse.createErrorResponse(error);
		}
	}

	async uploadAvatar(id: number, image: string): Promise<ServiceApiResponse<boolean>> {
		try {
			await this.db
				.update(users)
				.set({
