	...
	static sameSiteCookieConfig(): SameSiteCookieConfig {
		try {
			...
			if (isBlacklistedDomain(domainToCheck)) {
				// For blacklisted domains (like .onrender.com), use 'none' to allow cross-site requests
				// This is necessary for CSRF tokens to work when frontend and backend are on different subdomains
				// We don't set a domain to let the browser use the exact domain
				return {
					sameSite: "none",
					secure: true
					// Don't set domain for blacklisted domains to avoid subdomain issues
				};
			}
